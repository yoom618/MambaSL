...
if __name__ == "__main__":
    ...
    data_metainfo = "data_classification_ucr.yaml"
    ...
    # use train dataset as validation dataset (similar to InceptionTime setting)
    exp = "trainlossonly"
    model_configs = {
    ...
    for data_key, data_cfg in data_configs.items():
        ...
        replace_dict = {}
        if data_cfg["dataset"][0].lower() < "g":
            replace_dict["gpu"] = 0
        elif data_cfg["dataset"][0].lower() < "r":
            replace_dict["gpu"] = 1
        else:
            replace_dict["gpu"] = 2
        ...
        os.makedirs(f"{script_dir}/scripts_mamba/ucr/", exist_ok=True)
        with open(f"{script_dir}/scripts_mamba/ucr/MambaSL_UCR_{data_key}_{exp}.sh", "w") as f:
            f.write(scripts)