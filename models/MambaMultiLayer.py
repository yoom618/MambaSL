...
class Model(nn.Module):
    def __init__(self, configs):
        ...
        self.mamba = nn.Sequential(
            *[x for xs in lst for x in xs],  # flatten the list of lists
        ...
        # added for additional results in other tasks
    ...
    def forward(self, x_enc, x_mark_enc, x_dec=None, x_mark_dec=None, mask=None):
        ...
        # added for additional results in other tasks
            ...
            mamba_out = self.mamba(mamba_in)  # (B, L_in, D)
            out = self.out_layer(mamba_out)  # (B, L_in, D) -> (B, L_in, C_out)
            # out = out * x_mark_enc.unsqueeze(2)  # Mask out the padded sequence for the last data sample
